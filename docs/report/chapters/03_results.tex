...
Admin Dashboard cung cấp các chức năng quản trị hệ thống. Giao diện chia thành ba tabs: Trend Management, Keyword Approval, và Keyword Management.

\textbf{Trend Management:} Admin có thể xem tất cả trends với đầy đủ thông tin, tìm kiếm theo topic, và xóa trends không mong muốn. Mỗi trend hiển thị thêm thông tin kỹ thuật như trend\_id, created\_at, updated\_at. Chức năng xóa có confirmation để tránh xóa nhầm.

\textbf{Keyword Approval:} Hiển thị danh sách user submissions với status 'pending', bao gồm: keyword, platform, reason (lý do đề xuất), submitted\_by, submitted\_at. Admin có thể approve hoặc reject từng submission. Khi approve, keyword được thêm vào active\_keywords và submission status chuyển thành 'approved'. Khi reject, submission status chuyển thành 'rejected'. Hệ thống ghi log mỗi action vào keyword\_history.

\textbf{Keyword Management:} Hiển thị danh sách active\_keywords hiện tại, cho phép admin thêm keyword mới trực tiếp, xóa keyword không còn cần thiết, và xem keyword\_history (audit log). Keyword history ghi lại toàn bộ thay đổi: action (add/remove/approve/reject), keyword, performed\_by, timestamp.
